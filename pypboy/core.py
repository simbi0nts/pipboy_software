...
class Pypboy(game.core.Engine):
    ...
    def init_children(self):
        ...
        scanlines2 = pypboy.ui.Scanlines(800, 480, 8, 60,
                                         [(0, 10, 1, 0), (21, 62, 42, 90), (61, 122, 82, 100), (21, 62, 42, 90)] + [
                                             (0, 10, 1, 0) for x in range(50)], True)
    ...
    def init_gpio_controls(self):
        for pin in config.GPIO_ACTIONS.keys():
            print
            "Intialising pin %s as action '%s'" % (pin, config.GPIO_ACTIONS[pin])
    ...
    def switch_module(self, module):
	    ...
	    try:
                if self.active != self.modules[module]:
    ...
    def run(self):
        self.running = True
	timestamp = 0
	filename = '/home/bananapi/Desktop/pypboy/encoder_data.log'
        while self.running:
            for event in pygame.event.get():
                self.handle_event(event)
	    if os.path.exists(filename):
		flag = os.path.getmtime(filename)
		if timestamp != flag:
		    f = open(filename, 'r')
		    temp = f.read()
		    if temp == '0':
